	...
	<div class="row">
		<div class="col-md-4 mx-auto">
			...
			<form method="POST" action="{{ route('app_activation_account_change_email', ['token' => $token]) }}">
				...
				<div class="mb-3">
					<label for="new-email" class="form-label">New email adress</label>
					<input type="email" class="form-control @if(Session::has('danger')) is-invalid @endif" name="new-email" id="new-email" value="@if(Session::has('new_email')){{ Session::get('new_email') }}@endif" placeholder="enter the new email adress" required>
				</div>
